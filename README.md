...
Avoid `cat`-ing full data URI blobs into chat/tool output — wrapped/truncated copies are a common source of decode errors.

#### Flow B2 — local file path in Claude Code
If the MCP server runs locally and the file is on disk, call:

```json
{"image": "/full/path/to/file.jpg"}
```

This avoids large data-URI tool arguments entirely.
