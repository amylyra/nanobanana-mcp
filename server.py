...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "Every image tool result returns ImageContent objects that claude.ai can render inline.\n"
        "Tool metadata also includes `markdown` (or `markdown_gallery` for multiple images).\n"
        "In your assistant reply, include those markdown snippets so the generated image is visible in the chat response.\n"
...
def _build_image_response(
    ...
    - markdown: markdown snippet Claude can reuse to display the image in chat
    """
    ...
    for jpeg_bytes, meta in generated:
        ...
        if S3_BUCKET:
            ...
            meta["image_url"] = local_url
            meta["expires_in"] = "1 hour"
        meta["markdown"] = f"![generated image]({meta['image_url']})"
        # Use full generated bytes for preview so claude.ai shows the real output
        # instead of an aggressively downscaled thumbnail.
        previews.append(jpeg_bytes)
        markdown_lines.append(meta["markdown"])
        if save_folder:
    ...
    if len(generated) == 1:
        ...
        result["markdown_gallery"] = [meta.get("markdown") for _, meta in generated if meta.get("markdown")]
    result["render_hint"] = (
        "Include the markdown below in your assistant reply so the image is visible in chat."
    )
    ...
    return [Image(data=preview, format="jpeg") for preview in previews] + [json.dumps(result)]
